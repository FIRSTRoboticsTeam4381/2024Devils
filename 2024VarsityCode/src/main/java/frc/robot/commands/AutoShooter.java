...
public class AutoShooter extends Command {
  ...
  public AutoShooter(Pivot pivot, Shooter shooter, Limelight ll, Swerve swerve, boolean stopOnEnd) {
  ...
  private void calcAngle(){
    ...
    if(llDistance >= 0.3 && llDistance <= 9){ // Only take predicted position if it is a reasonable positive number. Prevents calculation of infinity.
      double calculatedAngle = 51.16053558 * Math.pow(predictedPosition, -0.5133988343)+1.25; // r^2 = 0.995 // despite predicting, still won't move unless a target is in sight. prevents going crazy
  ...
  public void end(boolean interrupted) {
    ...
    if(stopOnEnd) {shooter.setPercOutput(0.0, false);}
  }
