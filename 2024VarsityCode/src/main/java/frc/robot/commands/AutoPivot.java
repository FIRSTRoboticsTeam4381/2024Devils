...
public class AutoPivot extends Command {
  ...
  private void calcAngle(){
    ...
    if(ll.distanceFromGoal() >= 0.3){ // Only take predicted position if it is a reasonable positive number. Prevents calculation of infinity.
      double calculatedAngle = 55.62307316*Math.pow(predictedPosition, -0.5549909159627); // r^2 = 0.995 // despite predicting, still won't move unless a target is in sight. prevents going crazy
